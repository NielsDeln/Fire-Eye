...
def converge_gtow(
    ...
    for idx, m_pl in enumerate(payloads):
        #print(f"\n--- Drone {idx+1} ---")
        ...
        for i in range(max_iter):
            ...
            if abs(m_total - m0_guess) < tol:
                """
                print(f"Drone {idx+1} converged after {i+1} iterations.")
            ...
            raise RuntimeError(f"Drone {i+1}: GTOW did not converge.")
...
if __name__ == "__main__":
    ...
    print("\n\n========= FINAL SUMMARY =========")
    for i, (m_total, T_total, T_motor) in enumerate(results):
        print(f"\n--- Drone {i+1} ---")
        print(f"GTOW (m₀): {m_total:.2f} g")
        print(f"Total Thrust Required: {T_total:.2f} g")
        print(f"Per-Motor Thrust Required: {T_motor:.2f} g")
