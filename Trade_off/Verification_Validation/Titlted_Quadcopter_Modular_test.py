...
from Trade_off.Tilted_Quadcopter_Modular import modular_tquad

class TestModularTQuad(unittest.TestCase):
    def setUp(self):
        pass
    ...
    def test_m_payload(self):
        ...
        result = modular_tquad.m_payload(m_dmcomm, m_navig, m_mapping, m_control, m_forensics)
    ...
    def test_m_avionics(self):
        ...
        result = modular_tquad.m_avionics(m0)
        self.assertAlmostEqual(result, expected_result)
    ...
    def test_m_motor(self):
        ...
        result = modular_tquad.m_motor(T_motor)
        self.assertAlmostEqual(result, expected_result)
    
    def test_m_ESC(self):
        # Test the m_ESC function
        I_max = 30
        expected_result = (102.71*I_max+3510*4+2.2781*I_max**2+73.77*4**2+32.86*I_max*4)*10**(-6) *1000
        result = modular_tquad.m_ESC(I_max, 4)
        self.assertAlmostEqual(result, expected_result)
    ...
    def test_m_propeller(self):
        for d in [7.62, 10.16, 11.9, 15.4, 16]:
            ...
            result = modular_tquad.m_propeller(d)
            self.assertAlmostEqual(result, expected_result)
    ...
    def test_m_frame(self):
        ...
        result = modular_tquad.m_frame(D_UAV)
        self.assertAlmostEqual(result, expected_result)
    ...
    def test_GTOW(self):
        ...
        result = modular_tquad.GTOW(m_m, m_e, m_b, m_p, m_f, m_a, m_pl)
        self.assertAlmostEqual(result, expected_result)
