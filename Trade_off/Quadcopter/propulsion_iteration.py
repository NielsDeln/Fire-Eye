...
def select_best_motor_and_prop(GTOW, T_motor, motor_db1=motor_db):
    candidates = [m for m in motor_db1 if m['efficiency'] is not None]
    best_combo = None
...
def converge_gtow_and_prop(m_pl, battery_capacity=None, n_cells=None, tol=1e-2, max_iter=10, battery_override=None, n_batteries=None, motor_override=None):
    ...
    for i in range(max_iter):
        ...
        gtow, T_max, T_motor, m_m, m_e, m_b, m_p, m_f, m_a, m_pl = converge_gtow(m_pl, d_p=d_p, battery_cells=n_cells if n_cells is not None else 4, battery_capacity=battery_capacity, n_batteries=n_batteries, motor_override=motor_guess)
