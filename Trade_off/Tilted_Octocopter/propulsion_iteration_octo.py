...
from Trade_off.Tilted_Octocopter.weight_estimation_poop import converge_gtow, m_payload, m_pl
...
def select_best_motor_and_prop(T_motor):
    ...
    for motor in candidates:
        for d in prop_diameters:
            metric = evaluate_motor_prop_combo(motor, d, mass_aircraft=T_motor * 8)
...
def converge_gtow_and_prop(m_pl, battery_capacity=None, n_cells=None, tol=1e-2, max_iter=10, battery_override=None):
    ...
    for i in range(max_iter):
        ...
        gtow, T_max, T_motor = converge_gtow(m_pl, d_p=d_p, battery_cells=n_cells if n_cells is not None else 4, battery_capacity=battery_capacity if battery_capacity is not None else 5000, battery_override=battery_override if battery_override is not None else None, motor_override=motor_guess if battery_override is not None else None)
